...
async def request_funds(ctx: Context):
    await ctx.send(
        bob.address,
        PaymentRequest(wallet_address=str(ctx.wallet.address()),
                       amount=AMOUNT,
                       denom=DENOM)
    )
...
async def confirm_transaction(ctx: Context, sender: str, msg:TransactionInfo):
    ...
    """
     inherit this wait_for... class and coin recieved eventhas to fields amount and denom and string 
     concatenated
    """
...
async def send_payment(ctx: Context, sender: str, msg: PaymentRequest):
    ...
    transaction = ctx.ledger.send_tokens(msg.wallet_address, msg.amount, msg.denom, ctx.wallet) #sending payment
